...
from src.config import OPENAI_API_KEY, HT_TOKEN
...
class ModelHuggingFace:
    def __init__(self, model_name):
        model_dict = {
            ...
            'llama2-lat-layer2':'CurranZhai/llama2-lat-past-tense-1',
        ...
        self.model_name = model_name
        self.model = AutoModelForCausalLM.from_pretrained(model_dict[model_name], torch_dtype=torch.float16, device_map=self.device,token="", trust_remote_code=True).eval()
        self.tokenizer = AutoTokenizer.from_pretrained(model_dict[model_name], token="")
    def get_response(self, prompt, max_n_tokens, temperature):
...
class llama:
    def __init__(self,model_name):
        ...
        self.tokenizer = AutoTokenizer.from_pretrained("meta-llama/Meta-Llama-3.1-8B-Instruct", token="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")
        self.model = AutoModelForCausalLM.from_pretrained("meta-llama/Meta-Llama-3.1-8B-Instruct", torch_dtype=torch.float16, device_map=self.device,token="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", trust_remote_code=True).eval()
    def get_response(self, prompt, max_n_tokens, temperature):
        conv = [{"role": "user", "content": prompt}]
        prompt_formatted = self.tokenizer.apply_chat_template(conv, tokenize=False, add_generation_prompt=True)
        inputs = self.tokenizer(prompt_formatted, return_tensors='pt').to(self.device)
