...
def get_args():
    ...
    parser.add_argument('--dataset', type=str, default='anthropic-hh', help='Whether to use anthropic-hh or beavertails or past-tense')
...
if __name__ == '__main__':
    ...
    elif args.dataset=='past-tense':
        dataset_name = "past-tense-train"
        dataset = load_dataset(dataset_name).shuffle(seed=42)
        prompts = dataset['past-tense-train']['prompt']
        responses = dataset['past-tense-train']['response']
        is_safe = dataset['past-tense-train']['is_safe']
        prompts_test = dataset['past-tense-test']['prompt']
        responses_test = dataset['past-tense-test']['response']
        is_safe_test = dataset['past-tense-test']['is_safe']
    ...
    training_params = TrainingArguments(
        ...
        per_device_eval_batch_size=2,  # 减少批量大小
        gradient_accumulation_steps=16,  # 增加梯度累积步骤
        evaluation_strategy='steps',
        do_eval=True,
        eval_steps=0.25,  # 减少评估频率
        learning_rate=args.lr,
        weight_decay=0.0006,
        max_grad_norm=0.25,
        max_steps=-1,
        warmup_ratio=0.01,  # 调整warmup比例
        lr_scheduler_type='constant',
        fp16=True,  # 使用混合精度训练
    )
